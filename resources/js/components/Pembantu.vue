<template>
    <div class="container">
        <div class="row mt-5">
            <div class="col-md-3" v-for="pem in pembantu.data" :key="pem.id">
                <div class="card">
        ...
        <pagination :data="pembantu" @pagination-change-page="getResults"></pagination>
    </div>
</template>
...
<script>
    export default {
        ...
        methods: {
            loadPembantu(){
                axios.get("api/majikan/showPembantu").then(({data}) => (this.pembantu = data));
            },
            getFoto(a){
                return "storage/img/profile/" + a;
            },
            getResults(page = 1){
              axios.get('api/majikan/showPembantu?page=' + page)
                .then(response => {
                  this.pembantu = response.data;
                })
            },
        },
        created() {
